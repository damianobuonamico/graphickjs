interface Api {
  ...
  _on_wheel_event(target: number, deltaX: number, deltaY: number, ctrl: boolean): boolean;
  _on_clipboard_event(event: number): boolean;
  _on_touch_pinch(target: number, delta: number, center_x: number, center_y: number): boolean;
  ...
  _load_image(data: ArrayBuffer): void;
