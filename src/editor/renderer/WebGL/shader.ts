...
class Shader {
  ...
  constructor(gl: WebGL2RenderingContext | WebGLRenderingContext, id: string, source: string) {
    this.m_gl = gl;
    this.id = id;
  ...
  private createProgram(vertexShader: WebGLShader, fragmentShader: WebGLShader) {
    ...
    if (!gl.getProgramParameter(program, gl.LINK_STATUS)) {
      console.warn('Failed to link the program: ' + gl.getProgramInfoLog(program));
  ...
  public use() {
    ...
    if (this.m_uniformQueue.length) {
      for (let i = 0; i < this.m_uniformQueue.length; i++) {
        this.setUniform(this.m_uniformQueue[i].uniform, this.m_uniformQueue[i].value);
      }
  ...
  public setUniform(uniform: string, value: VectorOrMatrix) {
    ...
    vectorMatrixCallback({
      ...
      mat3: (value: mat3) => gl.uniformMatrix3fv(location, false, value),
