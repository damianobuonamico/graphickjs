  ...
  void Scene::render() const {
    ...
    for (auto entity : view) {
      if (m_registry.all_of<FillComponent>(entity)) {
        Renderer::Renderer::draw(view.get<PathComponent>(entity).path, view.get<TransformComponent>(entity).get_matrix(), m_registry.get<FillComponent>(entity).get_uint32());
      } else {
