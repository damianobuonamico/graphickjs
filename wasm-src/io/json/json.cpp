/**
 * @file io/json/json.cpp
 * @brief Contains the implementation of the JSON parser.
 */
...
JSON::JSON(const JSON& other)
{
  switch (other.m_type) {
    case Class::Object:
      m_internal.m_map = new std::map<std::string, JSON>(other.m_internal.m_map->begin(),
                                                         other.m_internal.m_map->end());
      break;
    case Class::Array:
      m_internal.m_list = new std::deque<JSON>(other.m_internal.m_list->begin(),
...
JSON& JSON::operator=(const rect& other)
{
  JSON box = JSON::array(JSON::array(other.min.x, other.min.y),
                         JSON::array(other.max.x, other.max.y));
  return operator=(box);
}
...
vec2 JSON::to_vec2(bool& ok) const
{
  ...
  if (ok) {
    ...
    if (ok) {
      return vec2{m_internal.m_list->at(0).to_float(), m_internal.m_list->at(1).to_float()};
    }
  }
...
vec3 JSON::to_vec3(bool& ok) const
{
  ...
  if (ok) {
    ...
    if (ok) {
      return vec3{m_internal.m_list->at(0).to_float(),
...
vec4 JSON::to_vec4(bool& ok) const
{
  ...
  if (ok) {
    ...
    if (ok) {
      return vec4{m_internal.m_list->at(0).to_float(),
                  m_internal.m_list->at(1).to_float(),
                  m_internal.m_list->at(2).to_float(),
