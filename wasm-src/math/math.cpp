...
namespace graphick::math {
  ...
  QuadraticSolutions<double> solve_quadratic(const double a, const double b, const double c, const bool include_double_roots) {
    if (is_almost_zero(a)) {
    ...
    if (is_almost_zero(discriminant)) {
      ...
      if (include_double_roots) return { root, root };
  ...
  QuadraticSolutions<double> solve_quadratic_normalized(const double a, const double b, const double c, const bool include_double_roots) {
    if (is_almost_zero(a)) {
      /* It is a linear equation */

      const double root = solve_linear(b, c);

      if (is_normalized(root)) return { root };
    ...
    const double root1 = (q - b) / a2;
    const double root2 = (-b - q) / a2;

    const bool root1_norm = is_normalized(root1);
    const bool root2_norm = is_normalized(root2);

    if (root1_norm && root2_norm) return { root1, root2 };
    else if (root1_norm) return { root1 };
    else if (root2_norm) return { root2 };

    return {};
  }

  CubicSolutions<double> solve_cubic(const double a, const double b, const double c, const double d, const bool include_double_roots) {
    if (is_almost_zero(a)) {
    ...
    if (is_almost_zero(d)) {
      ...
      if (!include_double_roots) {
        if (
          (solutions.count == 0) ||
          (solutions.count == 1 && solutions.solutions[0] != 0) ||
  ...
  CubicSolutions<double> solve_cubic_normalized(const double a, const double b, const double c, const double d, const bool include_double_roots) {
    if (is_almost_zero(a)) {
      /* It is a quadratic equation */
    ...
    if (is_almost_zero(discriminant)) {
      ...
      const uint8_t flag = (static_cast<uint8_t>(root1_norm)) | (static_cast<uint8_t>(root2_norm) << 1) | (static_cast<uint8_t>(root3_norm) << 2);

      switch (flag) {
      case 0b001: return { root1 };
      case 0b010: return { root2 };
      case 0b011: return { root1, root2 };
      case 0b100: return { root3 };
      case 0b101: return { root1, root3 };
      case 0b110: return { root2, root3 };
      case 0b111: return { root1, root2, root3 };
      default:
      case 0b000: return {};
      }
    }
