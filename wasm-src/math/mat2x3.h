 ...
 *
 * This matrix is not mathematically correct, it is only used to reduce the memory footprint of 2D
 * transforms. The missing row is always interpreted as [0, 0, 1]. When multiplying a Vec2<T> with
 * this matrix, the third component of the vector is always treated as 1.
 */
template<typename T>
struct Mat2x3 {
  /* -- Component accesses -- */
  ...
  constexpr Mat2x3(T x0, T y0, T z0, T x1, T y1, T z1)
      : value{Vec3<T>(x0, y0, z0), Vec3<T>(x1, y1, z1)}
  {
  }
  ...
  {
    return Mat2x3<T>(1, 0, v.x, 0, 1, v.y);
  }

  static constexpr Mat2x3<T> from_rotation(const T sin, const T cos)
  {
    return Mat2x3<T>(cos, -sin, 0, sin, cos, 0);
  }

  static constexpr Mat2x3<T> from_scale(const Vec2<T> &v)
  {
    return Mat2x3<T>(v.x, 0, 0, 0, v.y, 0);
  }

  static constexpr Mat2x3<T> from_translation_rotation_scale(const Vec2<T> &translation,
                                                             const T sin,
                                                             const T cos,
                                                             const Vec2<T> &scale)
  {
    return Mat2x3<T>(
        scale.x * cos, -scale.y * sin, translation.x, scale.x * sin, scale.y * cos, translation.y);
  }
...
constexpr Mat2x3<T> operator*(const Mat2x3<T> &m1, const Mat2x3<T> &m2)
{
  return Mat2x3<T>(m1[0][0] * m2[0][0] + m1[0][1] * m2[1][0],
