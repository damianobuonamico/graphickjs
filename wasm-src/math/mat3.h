...
struct Mat3 {
  ...
  constexpr explicit Mat3(T scalar) : value{Vec3<T>(scalar, 0, 0), Vec3<T>(0, scalar, 0), Vec3<T>(0, 0, scalar)} { }

  constexpr Mat3(const Vec3<T>& v0, const Vec3<T>& v1, const Vec3<T>& v2) : value{Vec3<T>(v0), Vec3<T>(v1), Vec3<T>(v2)} { }
  ...
  static constexpr Mat3<T> zero() { return Mat3<T>(T(0)); }

  static constexpr Mat3<T> identity() { return Mat3<T>(T(1)); }
  ...
  constexpr Mat3<T>& operator*=(const Mat3<T>& m) { return (*this = *this * m); }
