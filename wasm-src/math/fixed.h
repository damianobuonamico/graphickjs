 ...
 *
 * The f8x8 type is a 16 bit fixed point number with 8 bits for the integer part and 8 bits for the
 * fractional part. The f8x8x2 type is a 32 bit 2D vector of f8x8 numbers. The f8x8x4 type is a 64
 * bit 4D vector of f8x8 numbers.
 *
 * The f24x8 type is a 32 bit fixed point number with 24 bits for the integer part and 8 bits for
 * the fractional part. The f24x8x2 type is a 64 bit 2D vector of f24x8 numbers. The f24x8x4 type
 * is a 128 bit 4D vector of f24x8 numbers.
 */
...
struct Fixed {
  ...
  template<typename U>
  constexpr explicit Fixed(U x) : x(static_cast<Base>(x * FRACUNIT))
  {
  }

  template<typename T, typename I, uint8_t F>
  constexpr explicit Fixed(Fixed<T, I, F> x) : x(static_cast<Base>(x.x))
  {
  }
  ...
  constexpr Fixed<Base, Intermediate, Fractional> &operator+=(
      const Fixed<Base, Intermediate, Fractional> x)
  {
    this->x += x.x;
    return *this;
  }
  ...
  constexpr Fixed<Base, Intermediate, Fractional> &operator*=(
      const Fixed<Base, Intermediate, Fractional> x)
  {
    this->x = static_cast<Base>(
        (static_cast<Intermediate>(this->x) * static_cast<Intermediate>(x.x)) >> FRACBITS);
    return *this;
  }
  ...
  constexpr Fixed<Base, Intermediate, Fractional> &operator/=(
      const Fixed<Base, Intermediate, Fractional> v)
  {
    this->x = static_cast<Base>((static_cast<Intermediate>(this->x) << FRACBITS) /
                                static_cast<Intermediate>(v.x));
    return *this;
  }
...
constexpr Fixed<T, I, F> operator%(const Fixed<T, I, F> x, U scalar)
{
  return Fixed<T, I, F>(((x.x >> x.FRACBITS) % static_cast<T>(scalar))
                        << x.FRACBITS + x.frac_bits());
}
