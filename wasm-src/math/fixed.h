...
struct Fixed {
  ...
  constexpr Fixed int_bits() const { return (x >> FRACBITS) << FRACBITS; }

  constexpr Fixed frac_bits() const { return (x << INTBITS) >> INTBITS; }
  ...
  constexpr Base raw() const { return this->x; }

  constexpr float to_float() const { return static_cast<float>(*this); }

  constexpr double to_double() const { return static_cast<double>(*this); }
};
...
public:
  static inline graphick::math::Fixed<T, I, F> min() { return graphick::math::Fixed{numeric_limits<T>::min()}; }

  static inline graphick::math::Fixed<T, I, F> max() { return graphick::math::Fixed{numeric_limits<T>::max()}; }

  static inline graphick::math::Fixed<T, I, F> lowest() { return graphick::math::Fixed{numeric_limits<T>::lowest()}; }
};
