...
struct QuadraticSolutions {
  ...
  inline T operator[](const uint8_t i) const
  {
    return solutions[i];
  }
};
  ...
  CubicSolutions(const CubicSolutions<U> &cubic)
      : count(cubic.count),
        solutions{static_cast<T>(cubic.solutions[0]),
                  ...
                  static_cast<T>(cubic.solutions[2])}
  {
  }
...
/**
 * @brief Solves a quadratic equation of the form x^2 + ax + b = 0 and returns only normalized
 * solutions.
 *
 * @param a The coefficient of x^2.
 * @param b The coefficient of x.
 ...
 * @return A struct containing the normalized solutions to the equation.
 */
inline QuadraticSolutions<float> solve_quadratic_normalized(const float a,
                                                            const float b,
                                                            const float c)
{
  return solve_quadratic_normalized(
 ...
 * @return A struct containing the real solutions of the cubic equation.
 */
inline CubicSolutions<float> solve_cubic(const float a,
                                         const float b,
                                         const float c,
                                         const float d,
                                         const bool include_double_roots = false)
{
  return solve_cubic(static_cast<double>(a),
...
/**
 * @brief Solves a cubic equation of the form x^3 + ax^2 + bx + c = 0 and returns only normalized
 * solutions.
 *
 * @param a The coefficient of x^2.
 * @param b The coefficient of x.
 ...
 * @return A struct containing the normalized solutions of the cubic equation.
 */
inline CubicSolutions<float> solve_cubic_normalized(const float a,
                                                    const float b,
                                                    const float c,
                                                    const float d,
                                                    const bool include_coincident_roots = false)
{
  return solve_cubic_normalized(static_cast<double>(a),
                                static_cast<double>(b),
                                static_cast<double>(c),
                                static_cast<double>(d),
