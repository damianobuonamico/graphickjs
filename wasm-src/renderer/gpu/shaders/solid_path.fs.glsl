R"(
  ...
  float cubic_coverage() {
    ...
    vec2 pixel_pos = vTexCoord;
  
    uint band_index = clamp(uint(floor(pixel_pos.y * float(bands.x))), 0U, bands.x - 1U);
    ...
    float coverage = 0.0;
    float inv_pixel_size = 1.0 / pixel_size.x;

    vec2 position_delta = vPosition / vSize + pixel_pos;

    for (uint curve = 0U; curve < band_curves_count; curve++) {
      uint curve_offset = curves_index_offset + texture(uBandsTexture, to_coords(band_data_start + curve)).x * 2U;
