R"(
  ...
  flat in vec2 vSize;
  flat in uvec2 vBandsCount;
  flat in uint vCurvesStart;
  flat in uint vBandsStart;
  flat in uint vBandsDataStart;

  out vec4 oFragColor;

  const float kQuadraticEpsilon = 0.0001;

  vec2 index_to_coords(uint index) {
    return vec2(float(index % 512U), float(index / 512U)) / 512.0;
  }
  ...
  void main() {
    ...
    ivec2 sampleCount = ivec2(1, 1);

    // uvec2 glyphLoc = uvec2(0U, 0U);

    uvec2 bandIndex = uvec2(floor(vTexCoord * vec2(vBandsCount)));

    uint hBandDataStart = texture(uBandsTexture, index_to_coords(vBandsStart + bandIndex.x * 2U)).x + vBandsDataStart + uBandsDataStart;
    uint hBandCurvesCount = texture(uBandsTexture, index_to_coords(vBandsStart + bandIndex.x * 2U + 1U)).x;

    for (uint curve = 0U; curve < hBandCurvesCount; curve++) {
      uint curveIndex = texture(uBandsTexture, index_to_coords(hBandDataStart + curve)).x + vCurvesStart;


      vec2 curveLoc = index_to_coords(curveIndex);
      ...
      vec4 p12 = (texture(uCurvesTexture, curveLoc) - vec4(vPosition, vPosition)) / vec4(vSize, vSize) - vec4(vTexCoord, vTexCoord) - vec4(0.0, 0.5 / pixelsPerEm.y, 0.0, 0.5 / pixelsPerEm.y);
      vec2 p3 = (texture(uCurvesTexture, curveLoc + vec2(1.0, 0.0) / 512.0).xy - vPosition) / vSize - vTexCoord - vec2(0.0, 0.5 / pixelsPerEm.y);
      ...
      if (max(max(p12.x, p12.z), p3.x) * pixelsPerEm.x < -0.5) {
        oFragColor = vec4(0.0, 0.0, float(curveIndex) / 2.0, 1.0);
        return;
        continue;
      }
      ...
      if (code != 0U) {
        ...
        // oFragColor = vec4(0.0, float(curveIndex) / 2.0, 0.0, 1.0);
        // return;
      }
    }
    ...
    uint vBandDataStart = texture(uBandsTexture, index_to_coords(vBandsStart + (bandIndex.y + vBandsCount.x) * 2U)).x + vBandsDataStart + uBandsDataStart;
    uint vBandCurvesCount = texture(uBandsTexture, index_to_coords(vBandsStart + (bandIndex.y + vBandsCount.x) * 2U + 1U)).x;

    for (uint curve = 0U; curve < vBandCurvesCount; curve++) {
      uint curveIndex = texture(uBandsTexture, index_to_coords(vBandDataStart + curve)).x + vCurvesStart;
      vec2 curveLoc = index_to_coords(curveIndex);
      ...
      vec4 p12 = (texture(uCurvesTexture, curveLoc) - vec4(vPosition, vPosition)) / vec4(vSize, vSize) - vec4(vTexCoord, vTexCoord) - vec4(0.5 / pixelsPerEm.x, 0.0, 0.5 / pixelsPerEm.x, 0.0);
      vec2 p3 = (texture(uCurvesTexture, curveLoc + vec2(1.0, 0.0) / 512.0).xy - vPosition) / vSize - vTexCoord - vec2(0.5 / pixelsPerEm.x, 0.0);
