...
namespace Graphick::Render::GPU::GL {
  ...
  GLProgram GLDevice::create_program(const std::string& name) const {
    ...
    console::log("Created", gl_program);
  ...
  std::optional<GLUniform> GLDevice::get_uniform(const GLProgram& program, const std::string& name) const {
    console::log("Program", program.gl_program);
  ...
  void GLDevice::upload_to_texture(const GLTexture& texture, const Box& rect, const void* data) const {
  ...
  void GLDevice::draw_arrays(const uint32_t index_count, const GLRenderState& render_state) const {
  ...
  void GLDevice::draw_elements(const uint32_t index_count, const GLRenderState& render_state) const {
    set_render_state(render_state);
  ...
  GLShader GLDevice::create_shader(const std::string& name, const ShaderKind kind) const {
    ...
    std::string source = "#version " + m_glsl_version_spec + "\n" + ResourceManager::get_shader(name + (is_vertex ? ".vs" : ".fs"));
