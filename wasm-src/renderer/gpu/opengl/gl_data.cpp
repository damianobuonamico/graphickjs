                                        ...
                                        const VertexAttrDescriptor& desc) const
{
  ...
  if (desc.attr_class == VertexAttrClass::Int) {
    glCall(glVertexAttribIPointer(attr.attribute,
                                  (GLint)desc.size,
                                  attr_type,
                                  (GLsizei)desc.stride,
...
void GLTexture::set_sampling_flags(const int flags)
{
  ...
  glCall(glTexParameteri(GL_TEXTURE_2D,
                         GL_TEXTURE_WRAP_S,
                         flags & TextureSamplingFlagRepeatU ? GL_REPEAT : GL_CLAMP_TO_EDGE));
  glCall(glTexParameteri(GL_TEXTURE_2D,
                         GL_TEXTURE_WRAP_T,
    ...
    : texture(TextureFormat::RGBA8, size, TextureSamplingFlagNone)
{
  ...
  glCall(glFramebufferTexture2D(
      GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, texture.gl_texture, 0));
