...
namespace Graphick::Render::GPU {
  ...
  public:
     ...
     */
    inline static void upload_to_texture(const Texture& texture, const Box& rect, const void* data) { s_device->upload_to_texture(texture, rect, data); }
    /**
     ...
     */
    inline static void draw_arrays(const uint32_t index_count, const RenderState& render_state) { s_device->draw_arrays(index_count, render_state); }
    /**
     * @index_count: the number of indices to draw.
     * @render_state: the render state to use.
     * Performs a glDrawElements() call with the given @render_state.
     */
    inline static void draw_elements(const uint32_t index_count, const RenderState& render_state) { s_device->draw_elements(index_count, render_state); }
    /**
     * @index_count: the number of indices to draw.
     ...
     */
    inline static void draw_elements_instanced(const uint32_t index_count, const uint32_t instance_count, const RenderState& render_state) { s_device->draw_elements_instanced(index_count, instance_count, render_state); }
  private:
