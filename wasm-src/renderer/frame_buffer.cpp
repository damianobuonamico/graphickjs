...
void FrameBuffer::init() {
  ...
  glRenderbufferStorageMultisample(GL_RENDERBUFFER, samples, GL_RGBA8, m_size.x, m_size.y);
  ...
  glTexImage2DMultisample(GL_TEXTURE_2D_MULTISAMPLE, 8, GL_RGB, m_size.x, m_size.y, GL_TRUE);
  ...
  glGenRenderbuffers(1, &m_render_buffer_object);
  glBindRenderbuffer(GL_RENDERBUFFER, m_render_buffer_object);
  glRenderbufferStorageMultisample(GL_RENDERBUFFER, 8, GL_DEPTH24_STENCIL8, m_size.x, m_size.y);
...
void FrameBuffer::resize(const vec2& size) {
  ...
  glBindRenderbuffer(GL_RENDERBUFFER, m_render_buffer_object);
  glRenderbufferStorageMultisample(GL_RENDERBUFFER, 8, GL_DEPTH24_STENCIL8, size.x, size.y);
