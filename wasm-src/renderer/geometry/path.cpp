...
namespace Graphick::Render::Geometry {
  ...
  void Path::move_to(vec2 p) {
    m_last_point = std::make_shared<Vec2Value>(p);
  }

  void Path::line_to(vec2 p) {
    Segment::Point point = std::make_shared<Vec2Value>(p);
  ...
  void Path::arc_to(vec2 c, vec2 radius, float x_axis_rotation, bool large_arc_flag, bool sweep_flag, vec2 p) {
    float sin_th = std::sinf(degrees_to_radians(x_axis_rotation));
    float cos_th = std::cosf(degrees_to_radians(x_axis_rotation));
