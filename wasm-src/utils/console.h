...
private:
  ...
  struct TotalTimer {
     ...
     */
    inline void start() { last_time = std::chrono::high_resolution_clock::now(); }
     ...
     */
    inline void end(const size_t record) { records[index % RECORDS_SIZE] += record; }
     ...
     */
    inline size_t average() { return std::reduce(records.begin(), records.end()) / RECORDS_SIZE; }
  };
public:
  /**
   ...
   */
  static inline void time_start() { m_last_time = std::chrono::high_resolution_clock::now(); }
  ...
  static inline void time_end(const std::string& name = "Time Elapsed") {
    log(
      name,
      std::chrono::duration_cast<std::chrono::nanoseconds>(std::chrono::high_resolution_clock::now() - m_last_time).count()
    );
  }
  ...
  static inline void average_end(const std::string& name) {
    ...
    it->second.duration =
      (it->second.duration * it->second.samples + std::chrono::duration_cast<std::chrono::nanoseconds>(duration).count()) /
      (it->second.samples + 1);
    it->second.samples++;
...
#define GK_FRAME(name)                                                                                                           \
  graphick::utils::console::frame(name);                                                                                         \
  GK_TOTAL("MainThread")
#endif
