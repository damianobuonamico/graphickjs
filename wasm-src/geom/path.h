...
public:
   ...
   *
   * The iterator's value type is a tuple containing the command and the points of the segment (always 4 points, even if not
   * used).
   *
   * @struct Iterator
   */
  ...
  public:
     ...
     *
     * If index_type is IndexType::Point, and the point corresponds to a vertex, the iterator will point to the segment that ends
     * at the vertex.
     *
...
public:
   ...
   */
  inline Segment segment_at(const uint32_t index, const IndexType index_type = IndexType::Segment) const {
   ...
   * @param reverse Whether to add the point at the beginning of the path, instead of the end. Default is false.
   */
  inline void line_to(const geom::Line<T> line, const bool reverse = false) { line_to(line.p1, reverse); }
   ...
   * @return true if the path intersects the rect, false otherwise.
   */
  bool intersects(const math::Rect<T>& rect, const math::Mat2x3<T>& transform, std::vector<uint32_t>* indices = nullptr) const;
  ...
  /**
   * @brief Returns a new path that is the result of transforming the current path by the given matrix.
   *
   * Does not check if the transform is an identity matrix, should be done before calling this method.
