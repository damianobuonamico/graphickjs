 ...
 *
 * The following implementation is an adaptation of the algorithm in <https://github.com/aurimasg/cubic-bezier-offsetter/blob/main/C%2B%2B/CubicCurveOffset.cpp>.
 */
...
namespace graphick::geom {
   ...
   */
  static constexpr double arc_probe_positions[] = {
    0.2,
    0.4,
    0.6,
    0.8
   ...
   */
  static constexpr double simple_offset_probe_positions[] = {
    0.25,
    0.5,
    0.75
  };
  ...
  static void cubic_to(OutputBuilder<T>& builder, const dvec2 p1, const dvec2 p2, const dvec2 p3) {
    ...
    if (previous != p1 || previous != p2 || previous != p3) {
      ...
      builder.path.cubic_to(math::Vec2<T>(c1), math::Vec2<T>(c2), math::Vec2<T>(t), builder.cubic_tolerance);
  ...
  static bool accept_offset(
    const dcubic_bezier& original, const dcubic_bezier& parallel,
    ...
    if (
      geom::clockwise(original.p0, original.p1, original.p3) !=
    ...
    if (
      geom::clockwise(original.p0, original.p2, original.p3) !=
   ...
   */
  static bool DoubleArrayContainsMergePosition(
    const double* a, const int count,
  ...
  static int merge_curve_positions(
    double t[5], const int t_count,
    const double* s, const int count,
  ...
  static bool good_arc(
    const dvec2 arc_center, const double arc_radius,
  ...
  static bool try_arc_approximation(
    ...
    if (
      math::squared_distance(curve.p0, V) < (d.start_tangent.squared_length() * 0.25) ||
  ...
  */
  ...
  void offset_cubic(
    ...
    if (math::is_almost_zero(so)) {
      sink.cubic_to(math::Vec2<T>(curve.p1), math::Vec2<T>(curve.p2), math::Vec2<T>(curve.p3), T(2e-2));
      return;
    }
  ...
  template void offset_cubic(const dcubic_bezier&, const double, const double, QuadraticPath<float>&);
  template void offset_cubic(const dcubic_bezier&, const double, const double, QuadraticPath<double>&);
