...
inline bool is_point_in_ellipse(
...
inline bool is_point_in_rect(const math::Vec2<T> point, const math::Rect<T>& rect, const T threshold = T(0)) {
  return (
    point.x + threshold >= rect.min.x && point.x - threshold <= rect.max.x && point.y + threshold >= rect.min.y &&
...
inline bool is_point_in_rect(const math::Vec2<T> point, const math::Rect<T>& rect, const math::Vec2<T> threshold) {
  return (
    point.x + threshold.x >= rect.min.x && point.x - threshold.x <= rect.max.x && point.y + threshold.y >= rect.min.y &&
