...
namespace graphick::geom {
  ...
  template <typename T, typename = std::enable_if<std::is_floating_point_v<T>>>
  inline static math::Vec2<T> fast_cubic_first_solution(const math::Vec2<T> a, const math::Vec2<T> b, const math::Vec2<T> c, const math::Vec2<T> d) {
    /* d is guaranteed to be non-zero. */
    /* Calculate coefficients of the depressed cubic equation: y^3 + py + q = 0. */
  ...
  template <typename T, typename = std::enable_if<std::is_floating_point_v<T>>>
  inline static math::Vec4<T> fast_cubic_first_solution_plus_minus(const math::Vec2<T> a, const math::Vec2<T> b, const math::Vec2<T> c, const math::Vec2<T> d, const T tolerance) {
  ...
  template std::vector<std::pair<QuadraticBezier<float>, math::Vec2<float>>> cubic_to_quadratics_with_intervals(const CubicBezier<float>&);
  template std::vector<std::pair<QuadraticBezier<double>, math::Vec2<double>>> cubic_to_quadratics_with_intervals(const CubicBezier<double>&);
