...
namespace graphick::editor {
   ...
   *
   * Each scene has a selection manager that keeps track of the selected entities and children (e.g. vertices of an element).
   *
   * @class Selection
   */
  class Selection {
  public:
    ...
    struct SelectionEntry {
       ...
       */
      inline bool empty() const { return type == Type::Element && indices.empty(); }

      /**
       * @brief Checks if the selection entry is full.
       *
       * An entity selection entry is always full. If all the children of an element are selected, the element's selection is treated as an entity selection.
       *
       * @return true if the selection entry is full, false otherwise.
       */
      inline bool full() const { return type == Type::Entity; }
    };
  public:
    /**
     ...
     */
    inline bool empty() const { return m_selected.empty(); }
